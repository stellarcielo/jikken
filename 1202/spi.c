...
ch = 0;
while (ch >= 0) {
printf("mcp3008 からA/D 変換値を読み込みます。¥n");
...
printf(" ch%d = %d¥n",ch,ad_read(pd, fd, ch));
}
...
int val;
wdata[0] = ; // スタートビットになる
wdata[1] = ; // シングルエンド、チャンネルセット
spi_xfer( ); // SPI データの送受信
val = ; // 上位2 ビット分を取り込んでシフト
val = val + ; // 下位8 ビット分を取り込んで加算
return val;
}
